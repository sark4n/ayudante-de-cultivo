    ...
    return (
      <ul className="stats-report">
        <li className="empty-card">
          <i className="fas fa-lock" style={{ color: '#e53935', fontSize: '24px' }}></i>
          <p>¡Esta función está bloqueada!</p>
          <p className="text-left mt-2 text-green-700 dark:text-green-300">Tip del cultivador: Necesitas ser cultivador Experto para desbloquear esta función. ¡Sigue cultivando para conseguir más logros!</p>
        </li>
      </ul>
    );
  }
        ...
        return (
          <li key={index}>
            <div className="stats-card">
              <h3 className="plant-title"><i className="fas fa-leaf plant-icon"></i> {plant.name}</h3>
              <p><strong>Tipo:</strong> {plant.type}</p>
              <p><strong>Días viva:</strong> {daysAlive} días</p>
            </div>
            <div className="stats-card">
              <h4>Condiciones promedio</h4>
              <p><i className="fas fa-thermometer-half"></i> <strong>Temperatura:</strong> {avgTemp}°C</p>
              <p><i className="fas fa-tint"></i> <strong>Humedad:</strong> {avgHumidity}%</p>
            </div>
            <div className="stats-card">
              <h4>Días por etapa</h4>
              <div className="phase-bar"><div className="bar semilla" style={{ width: `${phaseDays.semilla}px` }}></div><span>Semilla: {phaseDays.semilla} días</span></div>
              <div className="phase-bar"><div className="bar vegetativa" style={{ width: `${phaseDays.vegetativa}px` }}></div><span>Vegetativa: {phaseDays.vegetativa} días</span></div>
              <div className="phase-bar"><div className="bar floracion" style={{ width: `${phaseDays.floracion}px` }}></div><span>Floración: {phaseDays.floracion} días</span></div>
            </div>
            <div className="stats-card locked">
              <h4>Predicción de cosecha</h4>
              <p className="locked-premium"><i className="fas fa-lock"></i> Desbloquea con Premium</p>
              <button className="premium-btn" onClick={() => setActiveSection('premium')}><i className="fas fa-star"></i> Obtener Premium</button>
            </div>
          </li>
        );
      })}
